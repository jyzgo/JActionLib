...
{
	public class JRotateMoveRoundTo : JFiniteTimeAction
	{
		...
		public JRotateMoveRoundTo (float duration, float radius, float radiusOri, Vector3 center, float startAngle, float angleDuration) : base (duration)
		{
			Duration = duration;
			Center = center;
			Radius = radius;
		...
		{
			return new JRotateMoveRoundToState (this, target);
		}
	...
	public class JRotateMoveRoundToState : JFiniteTimeActionState
	{

		public JRotateMoveRoundToState (JRotateMoveRoundTo action, GameObject target)
			: base (action, target)
		{ 
