...
{
	internal static class JEaseMath
	{
		// Sine Ease
		internal static float SineIn (float time)
		{
			return -1f * (float)Math.Cos (time * JMathHelper.Pi_2) + 1f;
		}

		internal static float SineOut (float time)
		{
			return (float)Math.Sin (time * JMathHelper.Pi_2);
		}
		...
		internal static float ElasticIn (float time, float period)
		{
			if (time == 0 || time == 1) {
				...
				float s = period / 4;
				time = time - 1;
				return -(float)(Math.Pow (2, 10 * time) * Math.Sin ((time - s) * JMathHelper.Pi * 2.0f / period));
			}
		}
		...
		internal static float ElasticOut (float time, float period)
		{
			if (time == 0 || time == 1) {
				...
				float s = period / 4;
				return (float)(Math.Pow (2, -10 * time) * Math.Sin ((time - s) * JMathHelper.Pi * 2f / period) + 1);
			}
		}
		...
		internal static float ElasticInOut (float time, float period)
		{
			if (time == 0 || time == 1) {
				...
				time = time - 1;
				if (time < 0) {
					return (float)(-0.5f * Math.Pow (2, 10 * time) * Math.Sin ((time - s) * JMathHelper.TwoPi / period));
				} else {
					return (float)(Math.Pow (2, -10 * time) * Math.Sin ((time - s) * JMathHelper.TwoPi / period) * 0.5f + 1);
				}
			}
		}
